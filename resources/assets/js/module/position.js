...
$(function () {
  let formPosition = $('#formPosition')
  let divSuccess = $('#form-message-success')
  let divError = $('#form-message-error')
  let positionDataTable = $('#position_table').dataTable({
    processing: true,
    serverSide: true,
    ajax: `${path}/getpager`,
    columns: [
      { data: 'name', name: 'position.name' },
      { data: 'description', name: 'position.description' },
      { data: 'owner', name: 'owner' },
      { data: 'updater', name: 'updater' },
      { data: 'created_at', name: 'position.created_at' },
      { data: 'updated_at', name: 'position.updated_at' },
  ...
  formPosition.validate({
    ...
    submitHandler: function (form) {
      let method = formPosition.find('input[name="_method"]').val()
      let url = path
      if (method === 'PUT') {
        let positionId = formPosition.find('input[name="position_id"]').val()
        url = `${path}/${positionId}`
      }
      $(form).ajaxSubmit({
        url: `${url}`,
        ...
        success: function (obj, statusText, xhr, $form) {
          ...
          resetForm(formPosition, ['_token', '_method'])
  ...
  $(document).on('click', '.positionEdit', function (ev) {
    ...
    .done(function (obj) {
      let data = obj.data
      setValue(formPosition, 'PUT')
      setValue(formPosition, data.id, 'position_id')
